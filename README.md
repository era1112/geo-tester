...
- "smarter" devices (ie. android handsets) quietly enter an undefined state, where the handset is unable to figure out where it is, but never throws a "gps loss" error. The device is also confused enough that it is unable to determine it's position through wifi location services.
